public class DES {
    ...
    //构造函数
    //mode = 0 加密   text:明文   key:密钥
    //mode = 1 解密   text:密文   key:密钥
    public DES(StringBuffer text, StringBuffer key, int mode) throws Exception {
        ...
        if(cipherText.length() % 8!= 0) throw new Exception("CipherText Error");
        if(key.length() < 8) throw new Exception("Key Error");
        this.key = key;
        encrypt_decrypt = mode;
    }
    //PKCS5填充处理明文
    private StringBuffer dealText(final StringBuffer origin) {
        ...
        for (int i = 0; i < max; ++i) {
            ...
            if(i>=origin.length()) charBinary = new StringBuffer(Integer.toBinaryString(padding));
            else charBinary = new StringBuffer(Integer.toBinaryString(origin.charAt(i)));
            while (charBinary.length() < 8) charBinary.insert(0, 0);
            textBinary.append(charBinary);
        }
        return textBinary;
    }
    //字符串转二进制
    private StringBuffer string2Binary(final StringBuffer origin) {
        StringBuffer textBinary = new StringBuffer();
    ...
    public void run() {
        ...
        if(encrypt_decrypt == 0)//使用PKCS#5/PKCS7填充
    ...
    public void IP(final String plainTextBinary) {
        ...
        for (int i = 0; i < 64; ++i) substituteplainText.append(plainTextBinary.charAt(IP[i] - 1));
        //把置换后的明文分为左右两块
    ...
    public void iterationT(StringBuffer L, StringBuffer R) {
        ...
        if(encrypt_decrypt != 0) {
            ...
            for (int i = 0; i < 16; ++i) subKey[i] = temp[15 - i];
        }
        ...
        for(int i = 0; i < 16; ++i) {
            ...
            for(int j=0;j<32;++j) R.replace(j,j+1,(tempL.charAt(j)==feistelString.charAt(j))?"0":"1");
        }
        //左右交换
        StringBuffer RL = new StringBuffer(R.toString() + L.toString());
        //逆置换
        StringBuffer substituteRL = new StringBuffer(); // 存储置换后的LR
        for (int i = 0; i < 64; ++i) substituteRL.append(RL.charAt(IPReverse[i] - 1));
        //删除多余字符
        if(group == 0) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if(encrypt_decrypt == 0) {
                ...
                while(tempHex.length() < 2) tempHex.insert(0, "0");
                cipherTextHex.append(tempHex + " ");
                //解密
            }else {
                if(group == 0 && 7 - i < Integer.parseInt(substituteRL.substring(56, 64), 2)) break;
    ...
    public StringBuffer feistel(final StringBuffer R, final StringBuffer subKey ) {
        ...
        for (int i = 0; i < 48; ++i) RExtent.append(R.charAt(E[i] - 1));
        //异或运算
        for (int i = 0; i < 48; ++i) RExtent.replace(i, i + 1, (RExtent.charAt(i) == subKey.charAt(i)) ? "0" :"1");
        //S-Box
        ...
        for(int i = 0; i < 8; ++i) {
            ...
            while (SBoxOutput.length() < 4) SBoxOutput.insert(0, 0);
            SBoxString.append(SBoxOutput);
        }
        //P置换
        StringBuffer substituteSBoxString= new StringBuffer(); // 存储置换后的R
        for (int i = 0; i < 32; ++i) substituteSBoxString.append(SBoxString.charAt(P[i] - 1));
    ...
    public StringBuffer[] getSubKey() {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            for (int j = 0; j < 48; ++j) C0D0Temp.append(C0D0.charAt(PC2[j] - 1));
            subKey[i] = C0D0Temp;
        }
        return subKey;
    }
    //返回密文
    public StringBuffer getcipherText() { return cipherText; }
    //返回密文十六进制
    public StringBuffer getcipherTextHex() { return cipherTextHex; }
    //返回明文
    public StringBuffer getplainText() { return plainText; }
}
