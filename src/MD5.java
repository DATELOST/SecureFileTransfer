public class MD5 {
    ...
    public String run(String message) {
        ...
        //分块
        for (int i = 0; i < groupCount; ++i) H(divide(inputBytes, i * 64));
        //填充
        int rest = byteLen % 64;
        //即将填充的一个分组
        byte[] paddingBytes = new byte[64];
        //原来的尾部数据
        for (int i = 0; i < rest; ++i) paddingBytes[i] = inputBytes[byteLen - rest + i];
        ...
        if (rest <= 56) {
            ...
            if (rest < 56) {
                ...
                //填充00000000
                for (int i = 1; i < 56 - rest; ++i) paddingBytes[rest + i] = 0;
            }
            //填充K值低64位
            for (int i = 0; i < 8; ++i) {
            ...
            paddingBytes[rest] = (byte) (1 << 7);
            //填充00000000
            for (int i = rest + 1; i < 64; ++i) paddingBytes[i] = 0;
            //处理第一个尾部分组
            H(divide(paddingBytes, 0));
            //填充00000000
            for (int i = 0; i < 56; ++i) paddingBytes[i] = 0;
            //填充低64位
            for (int i = 0; i < 8; ++i) {
    ...
    public static long byte2unsign(byte b) { return b < 0 ? b & 0x7F + 128 : b; }
    ...
    private void H(long[] groups) {
        ...
        result[3] += d;
        //防止溢出
        for (int n = 0; n < 4; n++) result[n] &= 0xFFFFFFFFL;
    }
}
